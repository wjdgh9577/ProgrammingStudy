...
#        명령을 내렸다. 조규현과 백승환은 각각 자신의 터렛 위치에서 현재
#        적까지의 거리를 계산했다.
#        조규현의 좌표 (x1, y1)와 백승환의 좌표 (x2, y2)가 주어지고,
#        조규현이 계산한 류재명과의 거리 r1과 백승환이 계산한 류재명과의
#        거리 r2가 주어졌을 때, 류재명이 있을 수 있는 좌표의 수를 출력하는
#        프로그램을 작성하시오.
...
for _ in range(int(input())):
    ...
    tri = sorted([((x1-x2)**2+(y1-y2)**2)**0.5, r1, r2])
    if tri[0] == 0:
        print(-1 if tri[1] == tri[2] else 0)
